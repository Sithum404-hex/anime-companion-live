...
export class AnimationController {
  ...
  update(dt: number) {
    ...
    // arms idle pose (slightly relaxed down)
    const targetArm = this.state === "HAPPY" ? 1 : this.state === "SCARED" ? 0.3 : 0;
    this.armBlend = THREE.MathUtils.lerp(this.armBlend, targetArm, Math.min(1, dt * 3));
    const setArm = (name: VRMHumanBoneName, restZ: number, happyZ: number) => {
      const b = hum.getNormalizedBoneNode(name);
      if (b) {
        const wave = this.state === "HAPPY" ? Math.sin(this.t * 6) * 0.2 : 0;
        b.rotation.z = THREE.MathUtils.lerp(restZ, happyZ + wave, this.armBlend);
      }
    };
    setArm(VRMHumanBoneName.LeftUpperArm, 1.25, 0.4);
    setArm(VRMHumanBoneName.RightUpperArm, -1.25, -0.4);
